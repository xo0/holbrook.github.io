...
从上面的安装测试操作中已经可以看出Salt的一些功能：批量查询状态，批量操作。尽管批量操作也很重要，但是如果仅仅是这个功能，使用Fabric一样能够做的很好。

Salt还有强大的配置管理、状态监控、命令编排、任务调度等神奇的功效，可以用来支持系统运维、应用开发、发布和部署等一系列的场景，是DevOps社区的一个热点工具。
